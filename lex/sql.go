...
import (
	...
	query "github.com/marianogappa/sqlparser/util"
)
...
func (p *parser) doParse() (query.Query, error) {
	...
	for {
		...
		case stepWhereOperator:
			...
			case "LIKE":
				currentCondition.Operator = query.Lk
			default:
		...
		case stepWhereValue:
			currentCondition := p.query.Conditions[len(p.query.Conditions)-1]
			identifier := p.peek()
			// log.Println("identifier:", identifier, isIdentifier(identifier))
			if isIdentifier(identifier) {
			...
			//judge and or
			if andorFlag == int(query.AND) {
				currentCondition.LogicalOperator = query.AND
			}
			if andorFlag == int(query.OR) {
				currentCondition.LogicalOperator = query.OR
			}
			p.query.Conditions[len(p.query.Conditions)-1] = currentCondition
			p.pop()
			p.step = stepWhereAndOR
		case stepWhereAndOR:
			Word := p.peek()
			if strings.ToUpper(Word) != "AND" && strings.ToUpper(Word) != "OR" {
				return p.query, fmt.Errorf("expected AND || OR")
			}
			// log.Println("Word:", Word)
			// log.Println(andorFlag)
			// currentCondition := p.query.Conditions[len(p.query.Conditions)-1]
			if strings.ToUpper(Word) == "AND" {
				andorFlag = int(query.AND)
			} else if strings.ToUpper(Word) == "OR" {
				andorFlag = int(query.OR)
			}
			p.pop()
			p.step = stepWhereField
